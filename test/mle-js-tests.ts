...
function testFetchTypeHandler() {
    const myFetchTypeHandler = function() {
        return {converter: (val: any) => val.toString()};
    };
    ...
    if (result.rows) {
        console.assert(result.rows[0][0] === '2', `${result.rows[0][0]} does not equal "2"`);
        console.log(result.rows[0][0]);
    }

    const myFetchTypeHandler2 = function(metaData: any) {
        if (metaData.dbType == oracledb.DB_TYPE_NUMBER) {
            return {
                converter: (val: string) => val + val,
    ...
    session.execute(sql, [], {
        outFormat: oracledb.ARRAY,
        fetchTypeHandler: myFetchTypeHandler2
    });
}
